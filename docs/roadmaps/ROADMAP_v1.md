...
- ✅ **Task 011: 더미 데이터 타입 정의 및 목업 구성** - 완료
...
- ✅ **Task 023: PDF 생성 및 다운로드 구현** - 완료
  - ✅ `@react-pdf/renderer` 라이브러리 선택 및 설치
...
- ✅ **Task 030: Notion API 응답 캐싱 최적화** - 완료
  - ✅ `unstable_cache`로 `getInvoiceById` 래핑 (revalidate: 300, tags: ['invoice'])
  - ✅ 페이지 레벨 ISR 적용 (`export const revalidate = 300`)
  - ✅ Rate Limit(429) 지수 백오프 재시도 (최대 3회, 1s→2s→4s)
  - See: `/tasks/030-caching-optimization.md`

- ✅ **Task 031: SEO 및 메타데이터 최적화** - 완료
  - ✅ `opengraph-image.tsx` - 동적 OG 이미지 생성 (1200x630, 견적서 번호/클라이언트명/금액)
  - ✅ `generateMetadata()` openGraph/twitter 카드 메타데이터 확장
  - ✅ `robots.ts` - /invoice/ Disallow (보안), `sitemap.ts` - 홈페이지 포함
  - See: `/tasks/031-seo-metadata.md`

- ✅ **Task 032: 접근성(a11y) 개선** - 완료
  - ✅ 견적서 테이블 `aria-label`, `scope="col"`, `role="list"` 추가
  - ✅ 상태 배지 `aria-label`, 날짜 `<time>` 태그 적용
  - ✅ URL 복사 버튼 `aria-live="polite"`, PDF 버튼 `aria-label` 추가
  - See: `/tasks/032-accessibility.md`

- ✅ **Task 033: Vercel 배포 파이프라인 구성** - 완료
  - ✅ `.github/workflows/ci.yml` - push/PR on main 시 자동 CI (Node 20, npm ci → check-all → build, NOTION secrets 주입)
  - ✅ `vercel.json` - framework: nextjs, buildCommand, 환경 변수 참조 설정
  - ✅ `.prettierignore`에 `shrimp_data/` 추가로 check-all 100% 통과
  - See: `/tasks/033-deployment.md`

- ✅ **Task 034: E2E 테스트 작성** - 완료
  - ✅ Playwright 기반 주요 흐름 테스트 (운영 환경 배포 후 실행 예정)
    - 견적서 조회 정상 흐름
    - 잘못된 URL 접근 시 404 처리
    - PDF 다운로드 흐름
