...
> 🚨 **테스트 절대 규칙 (예외 없음)**
>
> - **모든 구현 단계 완료 후 즉시 Playwright MCP 테스트 수행**
> - API 엔드포인트, 비즈니스 로직, UI 컴포넌트 구현 시 모두 해당
> - 테스트 실패 시 즉시 수정 후 재테스트 — 다음 단계 진행 절대 금지
> - 테스트 통과 확인 후에만 작업 파일 체크리스트 업데이트 가능
...
- [ ] 각 구현 단계마다 "구현 → 테스트 → 검증" 순서가 명시되어 있는가?
- [ ] Happy Path / Error Case / Edge Case 시나리오가 모두 포함되었는가?
- [ ] 테스트 실패 시 처리 절차(수정 후 재테스트)가 명확히 정의되었는가?
